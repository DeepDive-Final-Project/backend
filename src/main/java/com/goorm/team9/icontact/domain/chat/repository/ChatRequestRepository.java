...
public interface ChatRequestRepository extends JpaRepository<ChatRequest, Long> {
    ...
    @Query("SELECT cr FROM ChatRequest cr WHERE " +
            "cr.sender = :sender AND cr.receiver = :receiver " +
            "AND cr.status = 'PENDING'")
    Optional<ChatRequest> findPendingRequest(@Param("sender") ClientEntity sender,
                                             @Param("receiver") ClientEntity receiver);

    @Query("SELECT cr FROM ChatRequest cr WHERE cr.receiver = :receiver AND cr.status = :status")
    List<ChatRequest> findByReceiverAndStatus(@Param("receiver") ClientEntity receiver,
                                              @Param("status") RequestStatus status);

    @Query("SELECT cr FROM ChatRequest cr WHERE cr.sender = :sender AND cr.status = :status")
    List<ChatRequest> findBySenderAndStatus(@Param("sender") ClientEntity sender,
                                            @Param("status") RequestStatus status);

    @Query("SELECT COUNT(cr) " +
            "FROM ChatRequest cr " +
            "WHERE cr.receiver = :receiver AND cr.status = :status")
    long countReceivedRequests(@Param("receiver") ClientEntity receiver,
                               @Param("status") RequestStatus status);

    @Query("SELECT COUNT(cr) " +
            "FROM ChatRequest cr " +
            "WHERE cr.sender = :sender AND cr.status = :status")
    long countSentRequests(@Param("sender") ClientEntity sender,
                           @Param("status") RequestStatus status);
